...
func runBuild(cmd *command) (err error) {
	...
	if !nmpkgs["github.com/bilb02/go-mobile/app"] {
		return fmt.Errorf(`%s does not import "github.com/bilb02/go-mobile/app"`, pkg.ImportPath)
	}
...
func extractPkgs(nm string, path string) (map[string]bool, error) {
	if buildN {
		return map[string]bool{"github.com/bilb02/go-mobile/app": true}, nil
	}
	r, w := io.Pipe()
...
func importsApp(pkg *build.Package) error {
	...
	for _, path := range pkg.Imports {
		if path == "github.com/bilb02/go-mobile/app" {
			return nil
		}
	}
	return fmt.Errorf(`%s does not import "github.com/bilb02/go-mobile/app"`, pkg.ImportPath)
