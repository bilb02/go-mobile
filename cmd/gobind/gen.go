...
import (
	...
	"github.com/bilb02/go-mobile/bind"
	"github.com/bilb02/go-mobile/internal/importers"
	"github.com/bilb02/go-mobile/internal/importers/java"
	"github.com/bilb02/go-mobile/internal/importers/objc"
)
...
func genPkg(lang string, p *types.Package, astFiles []*ast.File, allPkg []*types.Package, classes []*java.Class, otypes []*objc.Named) {
	...
	case "java":
		...
		if p == nil {
			p, err := build.Default.Import("github.com/bilb02/go-mobile/bind", ".", build.ImportComment)
			if err != nil {
				errorf(`"github.com/bilb02/go-mobile/bind" is not found; run go get github.com/bilb02/go-mobile/bind: %v`, err)
				return
			}
			repo := filepath.Clean(filepath.Join(p.Dir, "..")) // github.com/bilb02/go-mobile directory.
			...
			// Copy support files
			javaPkg, err := build.Default.Import("github.com/bilb02/go-mobile/bind/java", "", build.FindOnly)
			if err != nil {
	...
	case "go":
		...
		io.Copy(w, &buf)
		closer()
		bindPkg, err := build.Default.Import("github.com/bilb02/go-mobile/bind", "", build.FindOnly)
		if err != nil {
	...
	case "objc":
		...
		if p == nil {
			// Copy support files
			objcPkg, err := build.Default.Import("github.com/bilb02/go-mobile/bind/objc", "", build.FindOnly)
			if err != nil {
